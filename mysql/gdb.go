...
var DURL = "root:123456@tcp(localhost:3306)/gdb?charset=utf8"
...
func updateUser() {
	db, err := sql.Open("mysql", DURL)
	if err != nil {
		fmt.Println(err)
	}
	//更新数据
	ret2, _ := db.Exec("update f_user set name = '000' where id = ?", 1)
	//获取影响行数
	num, _ := ret2.RowsAffected()
	fmt.Println(num)
}
...
func addUser() {
	...
	//插入一行数据
	ret, _ := db.Exec("insert into f_user(name,account,password) values('吕布','lvbu','123456')")
	//获取插入ID
...
func selectUser() {
	...
	rows, _ := db.Query("select * from f_user")

	// rows, err := db.Query("select name from f_user")
	// if err != nil {
	// 	log.Fatal(err)
	// }
	...
	for rows.Next() {
		var userID int32
		var name, account, password string
		if err := rows.Scan(&userID, &name, &account, &password); err != nil {
			log.Fatal(err)
		}
		fmt.Printf("%d   %s    %s   %s\r\n", userID, name, account, password)
	}
}
